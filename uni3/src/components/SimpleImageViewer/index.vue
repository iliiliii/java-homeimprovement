<template>
  <view v-if="visible" class="simple-image-viewer" @touchmove.stop.prevent>
    <!-- 背景遮罩 -->
    ...
    <view v-if="showThumbnail && images.length > 1" class="thumbnail-nav">
      <scroll-view scroll-x class="thumbnail-scroll" :scroll-into-view="'thumb-' + currentIndex">
...
const handleScale = (e) => {
  ...
  isScaled.value = e.detail.scale > 1.1 // 稍微放大就认为是缩放状态
}
